import type { DisplayPayload, Operator, ScientificFunction } from "./types.js";
import { Calculator } from "./calculator.js";

const previousValueElement = querySelector<HTMLSpanElement>("#previousValue");
const currentValueElement = querySelector<HTMLSpanElement>("#currentValue");
const historyListElement = querySelector<HTMLDivElement>("#historyList");
const errorIndicatorElement = querySelector<HTMLDivElement>("#errorIndicator");
const scientificToggleButton = querySelector<HTMLButtonElement>("#scientificToggle");
const themeToggleButton = querySelector<HTMLButtonElement>("#themeToggle");
const scientificPanel = querySelector<HTMLDivElement>("#scientificPanel");
...
function handleButtonClick(event: MouseEvent): void {
  ...
  switch (action) {
    ...
    case "decimal":
      ...
      break;
    case "operator":
      calculator.chooseOperator(value as Operator);
      break;
    case "clear":
      calculator.clear();
      break;
    ...
    case "equals":
      ...
      break;
      case "scientific":
      calculator.applyScientificFunction(value as ScientificFunction);
      break;
...
function playSound(type: "click" | "operator" | "error"): void {
...
function handleToggleScientificMode(): void {
...
function handleKeyboardInput(event: KeyboardEvent): void {
  ...
  if (["+", "-", "*", "/"].includes(key)) {
    event.preventDefault();
    calculator.chooseOperator(key as Operator);
    return;
  }
...
function handleStateChange(payload: DisplayPayload): void {
